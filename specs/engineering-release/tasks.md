...
_Última verificação documentada: release **0.9.1**._

## Cada alteração de produto

- [x] **T-REL-1 (S: aceitação 2–3, P: vsce)** `version` 0.9.1; `npm run vscode:package` / `npm run vsix` → `plantuml-viewer-0.9.1.vsix`.
- [x] **T-REL-2 (S: aceitação 3)** README actualizado com exemplos `.vsix` 0.9.1.

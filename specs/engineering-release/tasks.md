...
_Última verificação documentada: release **0.11.14** (ID `manorfm.plantuml-plugin-manorfm`, artefacto `plantuml-plugin-manorfm-0.11.14.vsix`)._

## Cada alteração de produto

- [x] **T-REL-1 (S: aceitação 2–3, P: vsce)** `version` 0.11.14; `main` → `./dist/extension.js`; `npm run vscode:package` / `npm run vsix` → `plantuml-plugin-manorfm-0.11.14.vsix`.
